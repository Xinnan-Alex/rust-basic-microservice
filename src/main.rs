...
const DEFAULT_DATABASE_URL: &str = "postgresql://postgres:0163626774@localhost:5432";
...
fn make_post_response(result: Result<i64, hyper::Error>,) -> FutureResult<hyper::Response, hyper::Error>{
    match result {
        Ok(timestamp) => {
            ...
            let response = Response::new().with_header(ContentLength(payload.len() as u64))
...
fn parse_form(form_chunk:Chunk) -> FutureResult<NewMessage, hyper::Error>{
    ...
    if let Some(message) = form.remove("message"){
        let username = form.remove("username").unwrap_or(String::from("annoymous"));
...
fn query_db(time_range: TimeRange, db_connection: &mut PgConnection) -> Option<Vec<Message>> {
    use schema::messages;
...
fn main() {
    ...
    let server = hyper::server::Http::new().bind(&address, ||Ok(Microservice{})).unwrap();
